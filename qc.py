...
class PneumaticControlGUI:
    ...
    def start_sequence(self):
        ...
        async def sequence_task():
            ...
            report_filename = f"auto_test_report_{timestamp}.txt"

            # Try to fetch IP up to 3 times
            ip = None
            ...
            all_steps = [(i+1, True) for i in range(5)] + [(i+1, False) for i in range(5)]
            for channel, open_valve in all_steps:
                success = False
                readback = {}
                for attempt in range(1, 4):
                    try:
                        self.root.after(0, lambda: self.message_label.config(
                            text=f"{'Open' if open_valve else 'Close'} Valve {channel} (Attempt {attempt}/3)"))
                        await pneumatic_set_valve(channel, open_valve)
                        readback = await pneumatic_read_valve_state(channel)
                        success = True
                        report_lines.append(f"Step: {'Open' if open_valve else 'Close'} Valve {channel} - Success on attempt {attempt}, State: {readback}")
                        break
                    except Exception as e:
                        await asyncio.sleep(0.5)
                if not success:
                    report_lines.append(f"Step: {'Open' if open_valve else 'Close'} Valve {channel} - Failed after 3 attempts")
                self.root.after(0, lambda: self._update_status("green" if success else "red"))
                await asyncio.sleep(0.5)
            ...
            self.root.after(0, lambda: self.message_label.config(text=f"Test complete. Report saved: {report_filename}"))
